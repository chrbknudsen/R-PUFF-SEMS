...
Så er der _en_ bestemt sandsynlighedsfordeling der optræder flere gange end andre.
...
:::: tiny
Det er _ikke_ en normativ betegnelse, normal i denne kontekst betyder blot den hyppigst forekommende
::::
...
pnorm(x) fortæller hvad sandsynligheden _p_ er for at se en observation der er mindre end _x_.
...
qnorm(p) fortæller os den værdi _x_, hvor sandsynligheden for at være mindre end _x_ er _p_. 
...
- Hvis vi ved hvad værdien/kvantilen er, bruger vi pnorm til at beregne sandsynligheden
...
:::: tiny
Når vi skalerer X til Z taler vi også om at vi beregner en _z-score_ for en given værdi af x. Det er et lækkert ord at bruge i en eksamensbesvarelse.
::::
...
En generel normalfordeling skriver vi som $N(\mu, \sigma)$  
Når data, X, er normalfordelt, skriver vi det som X ~ $N(\mu, \sigma)$  

I praksis:

P(X < 170) = P($\frac{X-\mu}{\sigma}$ < $\frac{170-\mu}{\sigma}$)
...
I hvilket interval ligger de så?
...
:::: {.columns}
:::: {.column}
For en normalfordeling gælder at $\sigma = 4.5$ og 75% af 
