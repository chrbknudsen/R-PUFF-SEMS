...
:::: {.tiny}
Prævalensen for TB kan vi også kalde risikoen for at have TB. Eller risk. Forholdet mellem to tal kalder vi også en ratio. Så i jeres branche kalder I det her for - tada - Risk Ratio. Eller RR.
::::
...
Hvor _er_ det bare pænt og symmetrisk! Og lige langt fra log(1) = 0
...
:::: {.fragment}
:::: {.tiny}
Og hvor ville verden dog være rar hvis de funktioner var indbyggede i R. Er verden rar? Nej, det er den ikke. Derfor findes den funktion heller ikke i R.
::::
::::
...
Vi laver de samme beregninger på de to andre celler. Og så tester vi om der er forskel på hvad vi ser. Og hvad vi forventer (hvis der ikke er forskel).
...
forventer, og hvad vi faktisk observerer, i data. Hvis der i virkeligheden ikke er forskel på fordelingen af TB-status i de to grupper.
::::
:::: {.fragment}
...
::::
:::: {.fragment}
Hvis vi placerer testværdien i normalfordelingen [link](The_Normal_Distribution.svg){target="_blank"}, så svarer det til en sandsynlighed.  
::::
:::: {.fragment}
...
::::
:::: {.fragment}
Eller, mere teknisk korrekt, sandsynligheden for at vi ser data der er så ekstreme som det er tilfældet (ekstreme relativt til $H_0$), hvis $H_0$ er sand.
Og selvom det er teknisk mere korrekt, er det her I bliver forvirrede, så hold jer til intuitionen om at det er risikoen for at begå en type-1 fejl.
::::
:::: {.fragment}
...
<tr class="fragment" data-fragment-index="9" style="border: none;">
...
Det er en af årsagerne til at den er lidt sjælden i eksamensopgaver. Men `chisq.test()` gør arbejdet for jer og finder den ved:  
`qchisq(1-0.05, df = 1)`
</div>
...
* De forventede værdier er meget små (<5) (det er de ikke her)
* grupperne er meget skæve (her er de fuldstændigt ens)

Man skal konstruere meget skæve, meget små datasæt for at opleve at de fire metoder er uenige om konklusionen.
</div>
