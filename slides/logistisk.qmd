...
:::: {.fragment}
Hvad er sandsynligheden for død (1) inden for de 3 år for de to grupper?
::::
:::: {.fragment}
...
:::: {.tiny}
Ser det umiddelbart ud til at der er en sammenhæng mellem dødeligheden (status 1: død) og vaccinestatus (dtp 1: vaccineret)?
...
::::
:::: {.fragment}
glm(status~vaegt + alder + factor(dtp) + factor(etngr), family = "binomial", data = data)
::::
...
_HBAAlC_dik_: 1 hvis HbAlc < 9%, 0 hvis HbAlc >= 9% 
_SRH_dik_: 1 hvis selvvurderet helbred er godt/meget godt. 0 hvis det er nogenlunde/dårligt/meget dårligt  
_AGE_: Alder i år  
_SEX_: Køn. 0: Kvinde, 1: Mand  
...
::::
:::: {.fragment}
Så hvad er den relative risiko for godt (selvvurderet) helbred (1), hvis HbAlc er 1 - altså velreguleret.
::::
::::
...
under operationen betydning for død i en opfølgningsperiode på ca. 3 år. 
...
Hvis vi alene betragter patienternes bmi, hvordan ville vi så undersøge om den har signifikant betydning for risikoen for dø efter 
operation.
...
::::
:::: {.fragment}
$SE\big(\log(OR)\big)=\sqrt{\frac{1}{a}+\frac{1}{b}+\frac{1}{c}+\frac{1}{d}}$
::::
...
:::: tiny
Husk nu. Det er odds, ikke risiko, for død der er højere ved det høje iltregime sammenlignet med det lave. Og selvom vi må konkludere at det ikke er signifikant,
er de glade for at nævne at det er tæt på at være signifikant.
::::
...
Med  s.e. på log(OR): sqrt(1/101 + 1/188 + 1/74 + 1/213) = 0.1828346
...
og se på log(OR) = sqrt(1/38 + 1/226 + 1/33 + 1/205) =  0.2567521
...
Undersøg om der er en statistisk signifikant sammenhæng mellem AG.positiv og status.
...
```{r levertal_reg_out_1, echo = FALSE}
library(tidyverse)
library(gt)
...
```{r levertal_reg_out_2, echo = FALSE}
library(tidyverse)
library(gt)
...
Hvilken skala er estimaterne på?
...
Prædikter sandsynligheden for at have status 1 (syg) for en AG.positiv kvinde på 50.


```{r levertal_reg_out_3, echo = FALSE}
library(tidyverse)
library(gt)
...
Variable:  
_Y_ er udfaldsvariablen (1: "treatment failure"),  
_R_ angiver behandling (1  = UDC),  
_X_ den standardiserede version af logaritmen af bilirubin (umol/L), målt for hver patient ved starten af studiet
...
Baseret på ovenstående tabel ønskes beregnet relativ risiko
...
```{r bilirubin_reg_output_1, echo = FALSE}
library(tidyverse)
library(gt)
...
::::
:::: {.fragment}
X er den uafhængige, kontinuærte variabel - den standardiserede logaritme af bilirubin målingerne
::::
:::: {.fragment}
...
```{r bilirubin_reg_output_2, echo = FALSE}
library(tidyverse)
library(gt)
...
::::
:::: {.fragment}
Ja, det kan debatteres. Tag dem alle sammen.
::::
::::
...
::::
:::: {.fragment}
En reduktion i _odds_ for treatment failure på ca. 60% ved ?
::::
:::: {.fragment}
