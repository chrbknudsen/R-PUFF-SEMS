...
Eller mere korrekt, at hvis vi beregner intervaller ud fra tilstrækkeligt mange stikprøver, så vil den sande værdi være indeholdt i 95% af de intervaller.
...
::::
:::: {.fragment}
Det er tosidet. Der kan $\mu$ være større eller mindre, vi er ligeglade og kun interesseret i om der er en forskel.
::::
:::: {.fragment}
Eller, ensidet:
...
$SE(\hat{\mu}_1 - \hat{\mu}_2) = \sqrt{SE_1^2 + SE_2^2} = \sqrt{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}}$
...
options(pillar.sigfig = 10)
tribble(~Gruppe, ~n, ~gennemsnit, ~spredning,
"Behandling", "n_1", 152.5, 18.4,
"Placebo", "n_0", 156.5, 19.1) 
```
::::
::::
...
::::
:::: {.fragment}
Så hvilken værdi er der blevet brugt ved et 90% KI?
::::
:::: {.fragment}
...
:::: tiny
Har I fået fingre i modelbesvarelsen - så er der regnefejl...
::::
...
df <- tibble(
  value = c(right_skew, left_skew,  log_right, log_left),
  type = rep(
    c(
      "Højreskæv",
      "Venstreskæv",
...
Beregn variationen på gennemsnittene (SEM) for
...
Beregn 95% KI for de danske og polske piger separat. Hvordan fortolker vi resultaterne? Kan vi konkludere noget om
...
::::
:::: {.fragment}
Det betyder _ikke_ at vi kan sige noget om en signifikant forskel eller ej.
::::
:::: {.fragment}
...
Udregn teststørrelsen for den approksimative t-test for sammenligning af log(D-vitaminniveua).
Brug de beregnede variationer på gennemsnittene. Brug teststørrelsen til at konkludere om forskellen på D-vitaminniveauet er statistisk signifikant på et 5% niveau.
...
Vi får nu at vide at vi kan beskrive log(D-vitaminniveau) i begge lande ved en normalfordeling.
Beregn et 95% reference interval for log(D-vit) for danske piger.
...
Brug derefter $\mu$ og $\sigma$ for de polske piger til at beregne hvor 
stor en andel af dem der forventes at ligge inden for det danske referenceinterval.

Hvis referenceintervallet for de danske piger er [nedre, øvre], hvordan finder vi
andelen af polske piger der ligger inden for det?
...
:::: {.fragment}
Hvad er det lig med? <span class="fragment">P(X < øvre) - P(X < nedre) </span>
::::

:::: {.fragment}
Hvordan får vi beregnet det? <span class="fragment">Vi normaliserer udtrykkene</span>
::::

:::: {.fragment}
Med hvad? <span class="fragment">$\mu$ og $\sigma$ for de polske piger</span>
::::
:::: {.fragment}
Og så skal vi bruge? <span class="fragment">pnorm</span>
::::

:::: {.fragment}
Fordi <span class="fragment">vi søger en andel/sandsynlighed.</span>
::::
